...
/**
 * Decides which business an invoice belongs to, from the lines it contains.
 *
 * Checkout used to hardcode 'combined' on every invoice, which made the report
 * filters useless: matchesBusiness() deliberately shows a combined invoice
 * under both the Cafe and the Burger filter, so with everything marked
 * combined, filtering by either returned every invoice ever taken.
 *
 * 'both' lines are shared menu items that either business can sell, so they
 * never decide the answer on their own — an order of nothing but shared items
 * is a cafe order. Only a genuine mix of the two, or a combined deal, is
 * 'combined'.
 */
export function invoiceBusinessTypeForLines(lines) {
  const present = new Set()
  for (const line of lines ?? []) {
    const t = line?.lineBusinessType
    if (t === 'combined') return 'combined'
    if (t === 'cafe' || t === 'burger') present.add(t)
  }
  if (present.size === 0) return 'cafe'
