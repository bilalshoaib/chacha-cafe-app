 ...
 */
export async function findExpenses(ctx, { from, to, businessType } = {}) {
...
export async function getExpenseById(ctx, id) {
  return withTenant(ctx, async (client) => {
    const res = await client.query(
...
export async function saveExpenses(ctx, expenses) {
      ...
      for (const e of expenses) {
        await client.query(
          `INSERT INTO expenses (id, title, amount, category, business_type, note, spent_at, created_at,
           ...
           WHERE expenses.tenant_id = EXCLUDED.tenant_id`,
          [
            e.id,
            e.title,
            e.amount,
            ...
            ctx.tenantId,
            e.locationId ?? ctx.locationId ?? null,
          ],
        )
      }
  })
...
export async function deleteExpenseById(ctx, id) {
  return withTenant(ctx, async (client) => {
    const res = await client.query(
