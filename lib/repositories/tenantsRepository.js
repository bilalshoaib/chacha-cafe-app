...
import { parseCurrency, localeForCurrency, isCurrency } from '../../constants/locales.js'
import { isTimezone, parseTimezone, DEFAULT_TIMEZONE } from '../../constants/timezones.js'
...
function rowToTenant(row) {
  ...
  return {
    ...
    // What they trade in. Selected from their first location by the queries
    // below, so the console's picker is filled from the same value the café's
    // own screens format with. The locale is derived from it rather than read
    // — see localeForCurrency — so the two cannot disagree.
    currency: parseCurrency(row.currency),
    locale: localeForCurrency(row.currency),
    // Where they are, which is what decides when their trading day ends. Read
    // from the same first location the currency is, and parsed rather than
    // trusted so a row holding a zone Intl cannot resolve still renders.
  ...
  const res = await pool.query(`
    SELECT t.*,
           ...
           (SELECT l.timezone FROM locations l
             WHERE l.tenant_id = t.id ORDER BY l.created_at LIMIT 1) AS timezone,
...
export async function updateTenant(id, fields) {
  ...
  // ── Currency ───────────────────────────────────────────────────────────
  // This lives on the location, not on the tenant, so it is a separate
  // statement — and it has to run before the early return below, which fires
  // when the only thing being changed is the currency.
  //
  // Written to every location of the tenant rather than just the first. A café
  // with two branches choosing dollars means the business trades in dollars;
  // the day one branch needs a currency of its own is the day a branch picker
  // ships, and that screen will set them one at a time.
  //
  // Refused rather than coerced. parseCurrency falls back to the default so a
  // stale row still renders, but accepting a bad code *into* the database that
  // way would silently move a café's prices to another currency.
  const locSets = []
  const locVals = []
  //
  // The locale column is written from the currency rather than taken from the
  // caller. Nothing offers a language to choose any more, and leaving the
  // column at whatever it held would leave a café moved to dollars still
  // writing its dates the Pakistani way for anything that reads the row
  // directly.
  if (fields.currency !== undefined) {
    if (!isCurrency(fields.currency)) return { error: 'That is not a currency this app can format.' }
    const code = parseCurrency(fields.currency)
    locVals.push(code); locSets.push(`currency = $${locVals.length}`)
    locVals.push(localeForCurrency(code)); locSets.push(`locale = $${locVals.length}`)
  }
  // The timezone travels with the currency, to every location for the same
  // reason and refused rather than coerced for the same reason. It is a
  // separate choice because the currency cannot supply it — `en-US` spans six
  // zones — and it is the more consequential of the two: it decides which
  // trading day a sale counts towards, so a wrong one moves takings between
  // days rather than merely printing the wrong symbol.
  if (fields.timezone !== undefined) {
    if (!isTimezone(fields.timezone)) return { error: 'That is not a timezone this app can read a clock in.' }
    locVals.push(fields.timezone); locSets.push(`timezone = $${locVals.length}`)
  }
  if (locSets.length) {
    locVals.push(id)
    await pool.query(
      `UPDATE locations SET ${locSets.join(', ')} WHERE tenant_id = $${locVals.length}`,
      locVals,
    )
    // The branding cache holds it, so the café sees its new currency on the
    // next navigation rather than up to fifteen seconds later.
    forgetTenantBranding(id)
  }
  ...
  // that changes whether this café may be used. The name, tagline, colours,
  // receipt footer and trading hours are in here too, so any write can also be
  // the one that changes how it looks.
...
export async function createTenant(input) {
  ...
  // Checked rather than trimmed to length, for the same reason the currency
  // below is: sixty characters of anything will fit in the column, but only
  // some of them are a zone `Intl` can read a clock in — and a café created
  // with a bad one would have every sale stamped with the wrong trading day
  // and nothing to say so.
  const timezone = input?.timezone === undefined ? DEFAULT_TIMEZONE : String(input.timezone)
  if (!isTimezone(timezone)) return { error: 'That is not a timezone this app can read a clock in.' }
  // Checked against the offered list rather than trimmed to length. Any three
  // characters will fit in the column, but only some of them are a currency,
  // and a café created with the rest formats every price through a fallback —
  // quietly, and in somebody else's money. Absent still means the default,
  // which is what the API's other callers rely on.
  if (input?.currency && !isCurrency(input.currency)) {
    return { error: 'That is not a currency this app can format.' }
  }
...
async function tenantRuntime(tenantId) {
  ...
  const res = await pool.query(
    `SELECT t.status, t.trial_ends_at, t.restricted_reason, t.day_start_hour, t.day_end_hour,
            (SELECT l.timezone FROM locations l
              WHERE l.tenant_id = t.id ORDER BY l.created_at LIMIT 1) AS timezone
  ...
  const tenant = row
    ? {
        ...
        // Joined in rather than fetched separately because this row is already
        // read on the way into every request and cached with the rest — the
        // till needs the zone at exactly the moment it needs the hours, to
        // stamp a sale with the day it belongs to.
        timezone: parseTimezone(row.timezone),
      }
    : null
...
/**
 * Deletes a café and everything that was only ever its own.
 *
 * A hard delete, guarded by the route: the café has to be suspended already —
 * which has signed everyone out and taken the public menu down — and the
 * platform owner has to have typed its name back. By the time this runs the
 * café is dark; this is the step that makes it gone.
 *
 * Two things are kept, both because the schema was built to keep them:
 *   - platform_payments — what the café paid the platform is the platform's
 *     record, not the café's. The row keeps the name copied onto it and loses
 *     its tenant_id (migration 024 left off ON DELETE CASCADE for exactly this).
 *   - audit_log — it carries no foreign key to tenants precisely so a deleted
 *     café does not take its trail with it. The caller writes the deletion there.
 *
 * The legacy `orders` table predates tenancy and has no tenant_id; untouched.
 *
 * Runs as the connecting role, so row level security does not apply. Child
 * tables that reference tenants without ON DELETE CASCADE are emptied first —
 * along with the brand-referencing rows that would otherwise block the brands
 * cascade — then the delete on tenants takes the rest: locations, brands,
 * memberships, categories, tenant_logos, shift_counters, invoice_counters,
 * shift_closes, tabs, tax_rates.
 */
export async function deleteTenant(id) {
  const found = await pool.query('SELECT name FROM tenants WHERE id = $1', [id])
  if (!found.rowCount) return { error: 'That café no longer exists.' }
  const name = found.rows[0].name

  await withTransaction(async (client) => {
    // Kept, unlinked. NULLs are distinct in the (tenant_id, period) unique
    // index, so every month's payment can lose its link in one statement.
    await client.query('UPDATE platform_payments SET tenant_id = NULL WHERE tenant_id = $1', [id])

    for (const table of [
      'deal_includes', 'deal_splits', 'invoices', 'expenses', 'deals', 'menu_items', 'users',
    ]) {
