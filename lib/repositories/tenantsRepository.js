...
import { parseCurrency, parseLocale, isCurrency, isLocale } from '../../constants/locales.js'
...
function rowToTenant(row) {
  ...
  return {
    ...
    // What they trade and read in. Selected from their first location by the
    // queries below, so the console's picker is filled from the same value the
    // café's own screens format with.
...
export async function updateTenant(id, fields) {
  ...
  // These two live on the location, not on the tenant, so they are a separate
  // statement — and it has to run before the early return below, which fires
  // when the only thing being changed is one of these two.
  //
  ...
  if (fields.currency !== undefined) {
    ...
    locVals.push(parseCurrency(fields.currency)); locSets.push(`currency = $${locVals.length}`)
  }
  if (fields.locale !== undefined) {
    if (!isLocale(fields.locale)) return { error: 'That is not a language this app offers.' }
    locVals.push(parseLocale(fields.locale)); locSets.push(`locale = $${locVals.length}`)
  }
  if (locSets.length) {
    locVals.push(id)
    await pool.query(
    ...
    // The branding cache holds both, so the café sees its new currency on the
...
export async function createTenant(input) {
  ...
  if (input?.locale && !isLocale(input.locale)) {
    return { error: 'That is not a language this app offers.' }
  }
