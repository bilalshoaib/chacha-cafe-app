 ...
 * used or not.
 *
 * setval is deliberately not used to hand any of them back. The sequence is
 * shared by every till in the café, so winding it backwards would re-issue
 * numbers another till is already printing on receipts.
 */
export async function reserveInvoiceNumbers(slug, count) {
  const allowed = ['cafe', 'burger', 'combined']
  if (!allowed.includes(slug)) throw new Error(`Unknown invoice slug: ${slug}`)
  const n = Math.max(1, Math.floor(Number(count) || 0))
  const res = await pool.query(
    `SELECT nextval($1) AS n FROM generate_series(1, $2)`,
    [`invoice_seq_${slug}`, n],
  )
