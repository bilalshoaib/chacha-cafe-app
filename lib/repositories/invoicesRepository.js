...
export async function countInvoices(filters = {}) {
  const { whereSql, values } = buildInvoiceWhere(filters)
  const res = await pool.query(`SELECT COUNT(*)::int AS total FROM invoices ${whereSql}`, values)
 ...
 */
export async function findInvoices(filters = {}, { limit = 20, offset = 0 } = {}) {
  const { whereSql, values } = buildInvoiceWhere(filters)
...
export async function getInvoicesInRange(from, to) {
  const res = await pool.query(
    'SELECT * FROM invoices WHERE created_at >= $1 AND created_at <= $2 ORDER BY created_at DESC',
    [from, to],
  )
  return res.rows.map(rowToInvoice)
}
