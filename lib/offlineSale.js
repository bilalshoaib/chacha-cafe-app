...
import { tableNumberForOrderType } from './tableNumber.js'
...
export function buildOfflineInvoice({
  ...
  const table = tableNumberForOrderType(tableNumber, type)
  ...
  // The hour and the zone both come from the reservation, so the till is
  // asking the same question the server asked when it handed the block over.
  // Without the zone this would be answered on Karachi's clock and a US till
  // would decide its shift had rolled over eleven hours early.
  const localShiftDate = Number.isFinite(Number(dayStartHour))
    ? shiftDateForInstant(createdAt, {
        shiftStartHour: Number(dayStartHour),
        ...(timezone ? { timezone } : {}),
      })
...
export function validateOfflineInvoice(raw) {
  ...
  const tableNumber = tableNumberForOrderType(raw.tableNumber, orderType)
  ...
  return {
    invoice: {
      ...
      ...(orderType ? { orderType } : {}),
      // Re-normalized rather than trusted, for the same reason the note is
      // re-truncated: a queue entry is client-written text, and the column is
