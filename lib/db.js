...
export async function withTenant(ctx, fn) {
  ...
  return withTransaction(async (client) => {
    ...
    //
    // A read-only context — support looking at a café's account without having
    // taken control of it — gets a role with no INSERT, UPDATE or DELETE at
    // all. Enforced here rather than by a check in each write route, because a
    // route guard has to be remembered on every route ever added.
    await client.query(ctx.readOnly ? 'SET LOCAL ROLE app_tenant_ro' : 'SET LOCAL ROLE app_tenant')
    return fn(client)
  })
}
