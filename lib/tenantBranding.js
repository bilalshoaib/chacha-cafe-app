...
import { DEFAULT_CURRENCY, DEFAULT_LOCALE, parseCurrency, parseLocale, directionOf } from '../constants/locales.js'
...
export const DEFAULT_BRANDING = {
  ...
  // What this café trades in and reads in. Both live on the location rather
  // than the tenant — a business can open across a border — and both travel
  // this road for the same reason the trading hours do: every screen that
  // shows a price needs them, on the first paint, and a second query and a
  // second provider to carry two short strings would be paid for on every
  // navigation.
...
export async function getTenantBranding(tenantId) {
  ...
  const branding = row
    ? {
        ...
        direction: directionOf(row.locale),
      }
    : DEFAULT_BRANDING
