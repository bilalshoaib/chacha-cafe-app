...
function buildReceiptHtml(invoice, itemLabelById) {
  ...
  const deliveryChargeLine = deliveryCharge > 0
    ...
    : ''
  return `<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"/><title>Receipt ${invoice.id}</title><style>@page{size:80mm auto;margin:3mm 5mm}*{box-sizing:border-box;margin:0;padding:0}body{font-family:'Courier New',Courier,monospace;font-size:10px;color:#000;width:100%;background:#fff}.center{text-align:center}.right{text-align:right}.bold{font-weight:bold}.dashed{border-top:1px dashed #000;margin:4px 0}.solid{border-top:1px solid #000;margin:4px 0}.shop-name{font-size:15px;font-weight:bold;text-align:center;letter-spacing:1px;margin-bottom:2px}.shop-sub{text-align:center;font-size:9px;margin-bottom:3px}.meta-row{font-size:9px;margin:1px 0}.meta-row span:first-child{font-weight:bold}.order-type-banner{text-align:center;font-size:14px;font-weight:bold;letter-spacing:1.5px;margin:5px 0;padding:4px 0;border-top:2px solid #000;border-bottom:2px solid #000}table{width:100%;border-collapse:collapse;margin:2px 0}th{font-size:9px;text-align:left;border-bottom:1px solid #000;padding:1px 0}th.amt-col{text-align:right}td{vertical-align:top;padding:2px 0;font-size:10px}.item-col{width:75%}.amt-col{width:25%;text-align:right;white-space:nowrap}.item-name{font-weight:bold}.item-each{font-size:9px;color:#333}.item-disc{font-size:9px;color:#555}.inc-line{font-size:9px}.total-row{display:flex;justify-content:space-between;font-size:12px;font-weight:bold;margin:3px 0}.status-row{text-align:center;font-size:10px;margin:2px 0}.footer{text-align:center;font-size:9px;margin-top:6px}.note-box{font-size:9px;margin:2px 0}.returned-notice{text-align:center;font-weight:bold;font-size:11px;border:1px solid #000;padding:2px 4px;margin:3px 0}</style></head><body><div class="shop-name">${businessName}</div><div class="shop-sub">Chacha Burger Cafe</div><div class="dashed"></div><div class="meta-row"><span>Invoice: </span>${invoice.id}</div><div class="meta-row"><span>Date: </span>${new Date(invoice.createdAt).toLocaleString()}</div>${invoice.orderId ? `<div class="meta-row"><span>Order: </span>${invoice.orderId}</div>` : ''}${orderTypeLine ? `<div class="order-type-banner">${orderTypeLine}</div>` : '<div class="dashed"></div>'}<table><thead><tr><th class="item-col">Item</th><th class="amt-col">Amt</th></tr></thead><tbody>${lineRows}</tbody></table><div class="solid"></div>${discountSummary}${!discountSummary ? deliveryChargeLine : (deliveryCharge > 0 ? `<div class="total-row" style="font-size:10px;font-weight:normal;"><span>Delivery Charge</span><span>${formatMoney(deliveryCharge)}</span></div>` : '')}<div class="total-row"><span>${invoice.paid || invoice.returned ? 'TOTAL' : 'TOTAL DUE'}</span><span>${formatMoney(invoice.total)}</span></div><div class="dashed"></div>${paymentLine ? `<div class="status-row">${paymentLine}</div>` : ''}<div class="status-row">${invoice.returned ? '** RETURNED **' : invoice.paid ? 'PAID' : 'UNPAID'}</div>${invoice.returned ? `<div class="returned-notice">** REFUNDED / RETURNED **</div>` : ''}${invoice.returnNote ? `<div class="note-box">Return note: ${invoice.returnNote}</div>` : ''}${invoice.customerNote ? `<div class="note-box">Note: ${invoice.customerNote}</div>` : ''}<div class="dashed"></div><div class="footer">Thank you for visiting!</div><div class="footer">Chacha Burger Cafe</div></body></html>`
}
